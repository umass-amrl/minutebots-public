...
#ifndef SRC_TACTICS_SETUP_ATTACKER_H_
#define SRC_TACTICS_SETUP_ATTACKER_H_
...
namespace tactics {
class SetupAttacker : public StateMachineTactic {
 public:
  SetupAttacker(const string& machine_name,
  ...
  const char* Name() const override { return "setup_attacker"; }
  void Init() override;
  void Reset() override;
 ...
 private:
  ...
  // Uses the other objects on the field to determine an appropriate position
  // to wait.
  Vector2f CalculateWaitPosition();
  ...
  const float kAttackerPadding = offense::kMinPassDistance;

  const float kRobotPadding = offense::kMinPassDistance;
  ...
  const float kTargetPadding = 1000;
  const float kSupportPadding = 2500;
};
}  // namespace tactics

#endif  // SRC_TACTICS_SETUP_ATTACKER_H_
