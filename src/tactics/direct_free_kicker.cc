...
DirectFreeKicker::DirectFreeKicker(const string& machine_name,
    ...
    : StateMachineTactic(machine_name, world_state, tactic_list, shared_state,
      ...
      setup_(std::bind(&DirectFreeKicker::Setup, this), "Start"),
...
void DirectFreeKicker::Init() {}

void DirectFreeKicker::Reset() {
  state_ = setup_;
  is_complete_ = false;
...
void DirectFreeKicker::SetGoal(const Pose2Df& pose) {}

bool DirectFreeKicker::IsComplete() { return is_complete_; }

bool DirectFreeKicker::ShouldKick(logger::Logger* logger,
...
float DirectFreeKicker::GetCost() {
...
void DirectFreeKicker::Setup() {
  ...
  GetTargetEvaluated(ball_pose, world_state_, soccer_state_, our_robot_index_,
                     false, &target_position, &target_angle_, &target_robot,
                     &last_target_);
...
void DirectFreeKicker::PostKick() {
  Tactic* controller = (*tactic_list_)[TacticIndex::HALT].get();
  controller->Run();
}

void DirectFreeKicker::Transition() {
  // Setup
  logger::Logger* robot_logger =
      soccer_state_->GetMutableRobotLoggerByOurRobotIndex(our_robot_index_);
