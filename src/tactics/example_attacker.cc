...
ExampleAttacker::ExampleAttacker(const string& machine_name,
    ...
    : StateMachineTactic(machine_name,
      ...
      aim_count_(0),
      kMaxAimIterations(3),
      start_(std::bind(&ExampleAttacker::Start, this), "Start"),
      catch_(std::bind(&ExampleAttacker::Catch, this), "Catch"),
      intercept_(std::bind(&ExampleAttacker::Intercept, this), "Intercept"),
      kick_(std::bind(&ExampleAttacker::Kick, this), "Kick"),
      post_kick_(std::bind(&ExampleAttacker::PostKick, this), "PostKick"),
      navigate_to_catch_(std::bind(&ExampleAttacker::NavigateCatch, this),
                         "NavigateToCatch"),
      navigate_to_intercept_(
          std::bind(&ExampleAttacker::NavigateIntercept, this),
          "NavigateToIntercept"),
      receive_(std::bind(&ExampleAttacker::Receive, this), "Receive"),
      stox_pivot_(std::bind(&ExampleAttacker::STOXPivot, this), "STOXPivot"),
  ...
  if (our_robot_index == 0) {
    ...
    data_file << "index,start_state,ball_velocity_x,ball_velocity_y,";
    data_file << "attacker_dist,catch_time,ball_time,angle_diff,angular_vel,";
    data_file << "radial_dist,rel_vel_y,rel_vel,y_dist,end_state" << endl;
  }
  LoadConfigFile();
}

void ExampleAttacker::SetValue(nlohmann::json config_json,
...
void ExampleAttacker::SetState(nlohmann::json config_json) {
...
void ExampleAttacker::LoadConfigFile() {
...
void ExampleAttacker::SetGoal(const Pose2Df& pose) {}

bool ExampleAttacker::IsComplete() { return is_complete_; }

bool ExampleAttacker::CouldBallScore(logger::Logger* the_logger,
...
bool ExampleAttacker::ShouldCatch(logger::Logger* the_logger,
                                  const float target_angle,
                                  const Vector2f current_ball_pose,
                                  const Vector2f current_ball_velocity,
                                  const Pose2Df current_robot_pose,
...
bool ExampleAttacker::ShouldKick(logger::Logger* logger,
                                 const float target_angle,
                                 const Vector2f current_ball_pose,
                                 const Vector2f current_ball_velocity,
                                 const Pose2Df current_robot_pose,
...
bool ExampleAttacker::DoesPathCrossBallRay() {
  float dist;
  Vector2f point;
...
bool ExampleAttacker::ShouldIntercept() {
  ...
  bool is_safe_path = !DoesPathCrossBallRay();
    ...
    return ball_speed_check && is_safe_path;
  return false;
}

bool ExampleAttacker::ShouldGoToBall() {
  // SET POTENTIAL TRANSITION
...
bool ExampleAttacker::ShouldSTOXPivot(
    const Vector2f current_ball_pose,
    const Vector2f current_ball_velocity,
    const Pose2Df current_robot_pose,
...
const Vector2f ExampleAttacker::GetNextInterceptPoint() {
...
const Pose2Df ExampleAttacker::GetGoal() {
  const Vector2f intercept_point = GetNextInterceptPoint();
  return {0, intercept_point};
}

void ExampleAttacker::GetSolution(Vector2f current_robot_trans,
                                  const Vector2f& current_velocity_world,
                                  const Vector2f& current_ball_pose,
                                  const Vector2f& current_ball_velocity) {
...
void ExampleAttacker::GetTarget(const Vector2f& source,
                                float* target_angle,
...
void ExampleAttacker::Catch() {
  Tactic* controller = (*tactic_list_)[TacticIndex::CATCH].get();
  controller->Run();
}

void ExampleAttacker::Intercept() {
  intercepting_ = true;
  Pose2Df current_pose =
      world_state_.GetOurRobotPosition(our_robot_index_).position;
  InterceptionController* controller = static_cast<InterceptionController*>(
...
void ExampleAttacker::NavigateCatch() {
...
void ExampleAttacker::NavigateIntercept() {
  Pose2Df current_pose =
      world_state_.GetOurRobotPosition(our_robot_index_).position;
  Vector2f ball_pose = world_state_.GetBallPosition().position;
...
void ExampleAttacker::PostKick() { Reset(); }

void ExampleAttacker::Receive() {
  Tactic* controller = (*tactic_list_)[TacticIndex::RECEIVER].get();
  controller->Run();
}

void ExampleAttacker::STOXPivot() {
  intercept_solution_.isInitialized = false;
  catch_solution_.isInitialized = false;
  logger::Logger* robot_logger =
      soccer_state_->GetMutableRobotLoggerByOurRobotIndex(our_robot_index_);
...
void ExampleAttacker::Transition() {
  // Setup
  logger::Logger* robot_logger =
      soccer_state_->GetMutableRobotLoggerByOurRobotIndex(our_robot_index_);
  ...
  data_file << count << ",";
  count++;
  data_file << state_.name_ << ",";
  data_file << std::setprecision(50);
  ...
  if (intersects) {
    const float attacker_dist =
    EuclideanDistance(intersect_point, robot_translation);
    data_file << std::setprecision(50) << 1 << "," << attacker_dist << ",";
  } else {
    data_file << std::setprecision(50) << -1 << "," << -1;
  }
  // ------ End Catch Variables ----------
  ...
  const Vector2f robot_to_ball_displace =
  current_ball_pose - current_pose.translation;
  Vector2f desired_norm(cos(target_angle_), sin(target_angle_));
  ...
  class::ThreeKick* three_kick_controller =
      static_cast<class ::ThreeKick*>(
          (*tactic_list_)[TacticIndex::THREE_KICK].get());

  const int tk_count = three_kick_controller->GetKickCount();

  class::Kick* kick_controller =
      static_cast<class ::Kick*>(
          (*tactic_list_)[TacticIndex::KICK].get());

  const int k_count = kick_controller->GetKickCount();

  const int kick_count =
    (k_count > tk_count) ? k_count : tk_count;

  // The boolean logic of "should_kick"
  const double angle_diff =
    RadToDeg(fabs(AngleDiff(target_angle_, current_pose.angle)));
  data_file << std::setprecision(50) << angle_diff << ",";
  const double angular_vel = RadToDeg(fabs(current_velocity.angle));
  data_file << std::setprecision(50) << angular_vel << ",";
  const double radial_dist = (collision_point - current_ball_pose).norm();
  data_file << std::setprecision(50) << radial_dist << ",";
  data_file << std::setprecision(50) << fabs(rel_y_prime_vel) << ",";
  const double relative_vel = (relative_velocity_vector).norm();
  data_file << std::setprecision(50) << relative_vel << ",";
  data_file << std::setprecision(50) << fabs(y_dist) << ",";
  data_file << std::setprecision(50) << fabs(kick_count) << ",";
  // ------ End Kick Variables ----------
