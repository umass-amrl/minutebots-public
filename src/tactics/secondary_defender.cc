...
SecondaryDefender::SecondaryDefender(const WorldState& world_state,
                                 TacticArray* tactic_list,
                                 SharedState* shared_state,
                                 OurRobotIndex our_robot_index,
                                 state::SoccerState* soccer_state)
    : StateMachineTactic("SecondaryDefender",
                         world_state,
                         tactic_list,
                         shared_state,
                         our_robot_index,
                         soccer_state),
      guard_(std::bind(&SecondaryDefender::Guard, this), "Guard"),
      intercept_(std::bind(&SecondaryDefender::Intercept, this), "Intercept"),
      intercepting_(false) {
  state_ = guard_;
  guard_pose_ = pose_2d::Pose2Df(0, 0, 0);
}

void SecondaryDefender::Init() {
  guard_pose_ = pose_2d::Pose2Df(0, 0, 0);
}

void SecondaryDefender::Guard() {
...
void SecondaryDefender::Intercept() {
  logger::Logger* robot_logger =
      soccer_state_->GetMutableRobotLoggerByOurRobotIndex(our_robot_index_);
...
void SecondaryDefender::Transition() {
  if (state_ == guard_) {
...
void SecondaryDefender::SetGoal(const Pose2Df& pose) { guard_pose_ = pose; }

bool SecondaryDefender::IsComplete() {
  return false;
}


float SecondaryDefender::GetCost() {
  Pose2Df current_pose =
      world_state_.GetOurRobotPosition(our_robot_index_).position;
  ...
  return DefenseEvaluator::CalculateSecondaryCost(
      world_state_.GetOurRobotPosition(our_robot_index_).ssl_vision_id,
      current_pose.translation,
      current_velocity);
}

void SecondaryDefender::SetIntercept() {
  intercepting_ = true;
}

void SecondaryDefender::Navigate(const ObstacleFlag& obstacles,
