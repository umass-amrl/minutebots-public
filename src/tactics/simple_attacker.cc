// Copyright 2018 - 2019 epruc@umass.edu ikhatri@umass.edu
...
SimpleAttacker::SimpleAttacker(const string& machine_name,
    ...
    : StateMachineTactic(machine_name,
      ...
      aim_count_(0),
      kMaxAimIterations(3),
      start_(std::bind(&SimpleAttacker::Start, this), "Start"),
      catch_(std::bind(&SimpleAttacker::Catch, this), "Catch"),
      intercept_(std::bind(&SimpleAttacker::Intercept, this), "Intercept"),
      kick_(std::bind(&SimpleAttacker::Kick, this), "Kick"),
      post_kick_(std::bind(&SimpleAttacker::PostKick, this), "PostKick"),
      navigate_to_catch_(std::bind(&SimpleAttacker::NavigateCatch, this),
                         "NavigateToCatch"),
      navigate_to_intercept_(
          std::bind(&SimpleAttacker::NavigateIntercept, this),
          "NavigateToIntercept"),
...
void SimpleAttacker::SetGoal(const Pose2Df& pose) {}

bool SimpleAttacker::IsComplete() { return is_complete_; }

bool SimpleAttacker::CouldBallScore(logger::Logger* the_logger,
...
bool SimpleAttacker::ShouldCatch(logger::Logger* the_logger,
                                  const float target_angle,
                                  const Vector2f current_ball_pose,
                                  const Vector2f current_ball_velocity,
                                  const Pose2Df current_robot_pose,
...
bool SimpleAttacker::ShouldKick(logger::Logger* logger,
                                 const float target_angle,
                                 const Vector2f current_ball_pose,
                                 const Vector2f current_ball_velocity,
                                 const Pose2Df current_robot_pose,
...
bool SimpleAttacker::DoesPathCrossBallRay() {
  float dist;
  Vector2f point;
...
bool SimpleAttacker::ShouldIntercept() {
  const Pose2Df current_pose =
      world_state_.GetOurRobotPosition(our_robot_index_).position;
...
bool SimpleAttacker::ShouldGoToBall() {
  // SET POTENTIAL TRANSITION
...
bool SimpleAttacker::ShouldSTOXPivot(
    const Vector2f current_ball_pose,
    const Vector2f current_ball_velocity,
    const Pose2Df current_robot_pose,
...
bool SimpleAttacker::ShouldNavigateToCatch() {
  return false;
  //   logger::Logger* robot_logger =
  //       soccer_state_->GetMutableRobotLoggerByOurRobotIndex(our_robot_index_);
  //   const Vector2f ball_vel = world_state_.GetBallPosition().velocity;
...
float SimpleAttacker::GetCost() {
  ...
  if (current_tactic == SIMPLE_ATTACKER) {
...
const Vector2f SimpleAttacker::GetNextInterceptPoint() {
...
const Pose2Df SimpleAttacker::GetGoal() {
  const Vector2f intercept_point = GetNextInterceptPoint();
  return {0, intercept_point};
}

void SimpleAttacker::GetSolution(Vector2f current_robot_trans,
                                  const Vector2f& current_velocity_world,
                                  const Vector2f& current_ball_pose,
                                  const Vector2f& current_ball_velocity) {
...
void SimpleAttacker::GetTarget(const Vector2f& source,
                                float* target_angle,
...
void SimpleAttacker::Catch() {
  Tactic* controller = (*tactic_list_)[TacticIndex::CATCH].get();
  controller->Run();
}

void SimpleAttacker::Intercept() {
  intercepting_ = true;
  Pose2Df current_pose =
      world_state_.GetOurRobotPosition(our_robot_index_).position;
  InterceptionController* controller = static_cast<InterceptionController*>(
...
void SimpleAttacker::NavigateCatch() {
...
void SimpleAttacker::NavigateIntercept() {
  Pose2Df current_pose =
      world_state_.GetOurRobotPosition(our_robot_index_).position;
  Vector2f ball_pose = world_state_.GetBallPosition().position;
...
void SimpleAttacker::PostKick() { Reset(); }

void SimpleAttacker::Receive() {
  Tactic* controller = (*tactic_list_)[TacticIndex::RECEIVER].get();
  controller->Run();
}

void SimpleAttacker::STOXPivot() {
  intercept_solution_.isInitialized = false;
  catch_solution_.isInitialized = false;
  logger::Logger* robot_logger =
      soccer_state_->GetMutableRobotLoggerByOurRobotIndex(our_robot_index_);
...
void SimpleAttacker::Transition() {
  // Setup
  logger::Logger* robot_logger =
      soccer_state_->GetMutableRobotLoggerByOurRobotIndex(our_robot_index_);
