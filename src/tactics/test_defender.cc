...
namespace tactics {
bool TestDefender::dump_file_opened_ = false;
FILE* TestDefender::file_ = NULL;

TestDefender::TestDefender(const WorldState& world_state,
                           TacticArray* tactic_list,
                           SharedState* shared_state,
                           OurRobotIndex our_robot_index,
                           state::SoccerState* soccer_state)
    : StateMachineTactic("TestDefender",
                         world_state,
                         tactic_list,
                         shared_state,
                         our_robot_index,
                         soccer_state),
      guard_(std::bind(&TestDefender::Guard, this), "Guard"),
      intercept_(std::bind(&TestDefender::Intercept, this), "Intercept"),
      intercepting_(false) {
  state_ = guard_;
  guard_pose_ = pose_2d::Pose2Df(0, 0, 0);

  if (kDumpData_) {
    string dump_file_name = "test_defender.csv";
    file_ = fopen(dump_file_name.c_str(), "w");
  }
}

void TestDefender::Init() {
  guard_pose_ = pose_2d::Pose2Df(0, 0, 0);
}

void TestDefender::Guard() {
  ...
  guard_pose_.translation = kTheta_ * (ball_pose - kOurGoalCenter).normalized()
                            + kOurGoalCenter;
...
void TestDefender::SetGoal(const Pose2Df& pose) { guard_pose_ = pose; }

bool TestDefender::IsComplete() {
  return false;
}


float TestDefender::GetCost() {
  Pose2Df current_pose =
      world_state_.GetOurRobotPosition(our_robot_index_).position;
...
void TestDefender::SetIntercept() {
  intercepting_ = true;
}

void TestDefender::Navigate(const ObstacleFlag& obstacles,
