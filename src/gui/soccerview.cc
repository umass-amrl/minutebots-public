// Copyright 2011-2018 joydeepb@cs.umass.edu
...
const float kDebugLineThickness = 3.0;
...
namespace gui {
const int GLSoccerView::kPreferedWidth = 1024;
const int GLSoccerView::kPreferedHeight = 768;

QSize GLSoccerView::sizeHint() const {
...
GLSoccerView::GLSoccerView(QWidget* parent)
    : QGLWidget(
      ...
      t_last_redraw_(0),
      selection_(SelectionType::kNoneSelected),
...
void GLSoccerView::DrawSelection() {
  if (selection_ == SelectionType::kBallSelected) {
  ...
  } else if (selection_ == SelectionType::kRobotSelected) {
...
void GLSoccerView::UpdateSelection(const Vector2f& loc) {
  graphics_mutex_.lock();
  ...
  for (const auto& robot : debug_msg_.world_state().robots()) {
    ...
    if (sq_dist < sq_min_dist) {
      ...
      sq_min_dist = sq_dist;
      selection_ = SelectionType::kRobotSelected;
    }
  }
  // Only select the ball if no robot has been found.
  if (selection_ == SelectionType::kNoneSelected) {
    for (int i = 0; i < debug_msg_.world_state().balls_size(); ++i) {
      ...
      if (sq_dist < sq_min_dist) {
        selection_ = SelectionType::kBallSelected;
...
Vector2f GLSoccerView::ScreenToWorld(const Vector2f& p_screen) const {
...
Vector2f GLSoccerView::WorldToScreen(const Vector2f& p_world) const {
...
void GLSoccerView::DrawEllipses() {
  const float line_width = 0.5 * kDebugLineThickness * view_scale_;
  // An epsilon to add to the Z-ordering so that alpha blending is performed
  // correctly.
  const float epsilon =
...
void GLSoccerView::DrawQuad(const Vector2f& loc1, const Vector2f& loc2,
                            float z) {
  glBegin(GL_QUADS);
...
void GLSoccerView::CallRobotDisplayList(Team team) {
  switch (team) {
    case Team::BLUE: {
...
void GLSoccerView::CompileRobotDisplayList(Team team) {
...
void GLSoccerView::DrawRobot(const Vector2f& loc, float theta, float conf,
...
void GLSoccerView::DrawRobotOutline(
    const Vector2f& loc,
    float theta,
    int robot_id,
    Team team) {
...
void GLSoccerView::DrawFieldLines() {
  SetColor(Color4f::kWhite);

  // Field boundary.
  DrawQuad(Vector2f(-kHalfFieldLength, kHalfFieldWidth - kFieldLineWidth),
           Vector2f(kHalfFieldLength, kHalfFieldWidth), kFieldZ);
  DrawQuad(Vector2f(-kHalfFieldLength, -kHalfFieldWidth),
           Vector2f(kHalfFieldLength, -kHalfFieldWidth + kFieldLineWidth),
           kFieldZ);
  DrawQuad(Vector2f(-kHalfFieldLength, -kHalfFieldWidth),
           Vector2f(-kHalfFieldLength + kFieldLineWidth, kHalfFieldWidth),
           kFieldZ);
  DrawQuad(Vector2f(kHalfFieldLength - kFieldLineWidth, -kHalfFieldWidth),
           Vector2f(kHalfFieldLength, kHalfFieldWidth), kFieldZ);

  // Field mid line and halfway line.
  DrawQuad(Vector2f(-kHalfFieldLength, -0.5 * kFieldLineWidth),
           Vector2f(kHalfFieldLength, 0.5 * kFieldLineWidth), kFieldZ);
  DrawQuad(Vector2f(-0.5 * kFieldLineWidth, -kHalfFieldWidth),
           Vector2f(0.5 * kFieldLineWidth, kHalfFieldWidth), kFieldZ);

  // Center circle.
  DrawArc(Vector2f(0, 0), kCenterCircleRadius - kFieldLineWidth,
          kCenterCircleRadius, -M_PI, M_PI, kFieldZ, kFieldLinesColor);

  // Left defense area.
  DrawQuad(Vector2f(-kHalfFieldLength, -kDefenseStretch),
           Vector2f(-kHalfFieldLength + kDefenseStretch,
                    -kDefenseStretch + kFieldLineWidth), kFieldZ);
  DrawQuad(Vector2f(-kHalfFieldLength, kDefenseStretch - kFieldLineWidth),
           Vector2f(-kHalfFieldLength + kDefenseStretch,
                    kDefenseStretch), kFieldZ);
  DrawQuad(Vector2f(-kHalfFieldLength + kDefenseStretch - kFieldLineWidth,
                    -kDefenseStretch),
           Vector2f(-kHalfFieldLength + kDefenseStretch,
                    kDefenseStretch), kFieldZ);
  ...
  // Right defense area.
  DrawQuad(Vector2f(kHalfFieldLength -kDefenseStretch, -kDefenseStretch),
           Vector2f(kHalfFieldLength, -kDefenseStretch + kFieldLineWidth),
           kFieldZ);
  DrawQuad(Vector2f(kHalfFieldLength -kDefenseStretch,
                    kDefenseStretch - kFieldLineWidth),
           Vector2f(kHalfFieldLength, kDefenseStretch), kFieldZ);
  DrawQuad(Vector2f(kHalfFieldLength - kDefenseStretch,
                    -kDefenseStretch),
           Vector2f(kHalfFieldLength - kDefenseStretch + kFieldLineWidth,
                    kDefenseStretch), kFieldZ);
  ...
  // Left goal.
  DrawQuad(
      Vector2f(-kHalfFieldLength - kGoalDepth - 20.0, -0.5 * kGoalWidth - 20.0),
      Vector2f(-kHalfFieldLength - kGoalDepth, 0.5 * kGoalWidth + 20.0),
      kFieldZ);
  DrawQuad(Vector2f(-kHalfFieldLength - kGoalDepth, -0.5 * kGoalWidth - 20.0),
           Vector2f(-kHalfFieldLength, -0.5 * kGoalWidth), kFieldZ);
  DrawQuad(Vector2f(-kHalfFieldLength - kGoalDepth, 0.5 * kGoalWidth),
           Vector2f(-kHalfFieldLength, 0.5 * kGoalWidth + 20.0), kFieldZ);

  // Right Goal
  DrawQuad(
      Vector2f(kHalfFieldLength + kGoalDepth, -0.5 * kGoalWidth - 20.0),
      Vector2f(kHalfFieldLength + kGoalDepth + 20, 0.5 * kGoalWidth + 20.0),
      kFieldZ);
  DrawQuad(Vector2f(kHalfFieldLength, -0.5 * kGoalWidth - 20.0),
           Vector2f(kHalfFieldLength + kGoalDepth, -0.5 * kGoalWidth), kFieldZ);
  DrawQuad(Vector2f(kHalfFieldLength, 0.5 * kGoalWidth),
           Vector2f(kHalfFieldLength + kGoalDepth, 0.5 * kGoalWidth + 20.0),
           kFieldZ);
}

void GLSoccerView::DrawBall(const Vector2f& loc) {
  DrawArc(loc, 0, 16, -M_PI, M_PI, kBallZ, kBallColor);
  DrawArc(loc, 15, 21, -M_PI, M_PI, kBallZ, kFieldLinesColor);
}

void GLSoccerView::Update(const MinuteBotsProto::SoccerDebugMessage& msg) {
  graphics_mutex_.lock();
  debug_msg_ = msg;
  graphics_mutex_.unlock();
  RedrawSignal();
