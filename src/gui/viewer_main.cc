  ...
  void run() {
    ...
    while (true) {
      ...
      if (server_.TryReceiveProtobuf(&soccer_debug)) {
        ...
        bool kDebug = false;
        if (soccer_debug.team_yellow() == team_yellow) {
...
int main(int argc, char** argv) {
  ...
  server_.Open(DATA_STREAM_DEBUG_IP, DATA_STREAM_DEBUG_PORT, true);
  ...
  if (stat("viewer_logs/", &sb) == 0) {
    ...
    std::cout << "viewer_logs directory does not exist" << std::endl;
  }
  return 1;
}
