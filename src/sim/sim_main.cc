...
std::normal_distribution<double> rot_distribution(0.0, 0.0);
...
std::vector<Eigen::Vector2f> ReadStartPositions(const std::string& file_path) {
...
WorldState GenerateWorldState(int argc, char** argv) {
  if (argc > 1) {
    // Grab positions file.
    const std::vector<Eigen::Vector2f> positions = ReadStartPositions(argv[1]);
    CHECK_EQ(positions.size(), kNumRobots);
    return {positions, kTimeSlice};
  } else {
    return {kNumRobots, kTimeSlice};
  }
}
...
int main(int argc, char** argv) {
  ...
  WorldState world_state = GenerateWorldState(argc, argv);
