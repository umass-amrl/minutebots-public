...
// ========================================================================
#include "configuration_reader/lua_script.h"
#include <string.h>
...
void LuaScript::printError(const std::string& variableName,
  ...
  std::cerr << "Error: can't get [" << variableName << "]. " << reason
            << std::endl;
}
...
Eigen::Vector2f LuaScript::getVector2f(const std::string& name) {
  ...
  lua_gettostack(name.c_str());
  if (lua_isnil(L, -1)) {  // array is not found
    std::cerr << "Vector2f not found in Lua file" << std::endl;
    return v;
  }
  lua_pushnil(L);
  int i = 0;
  while (lua_next(L, -2) && i < 2) {
    if (strncmp(lua_typename(L, lua_type(L, -2)), "string", 6) == 0) {
      if (strncmp(lua_tostring(L, -2), "x", 1) == 0) {
        v(0) = (static_cast<float>(lua_tonumber(L, -1)));
      } else {
        v(1) = (static_cast<float>(lua_tonumber(L, -1)));
      }
      lua_pop(L, 1);
      i++;
    } else {
