...
int main(int argc, char** argv) {
  ...
  for (int x = 100; x < 4000; x += (kBallRadius * 3)) {
    for (int y = 2500; y > 500; y -= (kBallRadius * 3)) {
      int count = 0;
      int total = 0;
      for (float r = 0; r < 360; r += 20) {
        ...
        Eigen::Vector2f left(0, 0);
        Eigen::Vector2f right(kFieldXMax, 0);
        float dist = 0;
        Eigen::Vector2f intersect;
        bool intersects = geometry::RayIntersect(ball_loc, rotated, left, right,
                                                 &dist, &intersect);

        const bool bad_point = !intersects;

        if (!bad_point) {
          std::cout << "Doing Sim" << std::endl;
          NetworkHandler receiver(&world_state, DATA_STREAM_CMD_IP,
                                  atoi(argv[8]), DATA_STREAM_VISION_IP,
                                  atoi(argv[9]), kUpdateRate, kOutputRate,
                                  kReceiveRate);
          //           system("./bin/soccer -tb -py&");
          receiver.Start();
          while (true) {
            if (receiver.DeflectionSuccess()) {
              count += 1;
              total += 1;
              break;
