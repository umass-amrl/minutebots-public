...
#ifndef SRC_EXPERIMENTAL_SIM_OBJECTS_BALL_H_
#define SRC_EXPERIMENTAL_SIM_OBJECTS_BALL_H_
...
class Ball {
 public:
  Ball(const SimState& world_state,
       const pose_2d::Pose2Df initial_pose,
       const Eigen::Vector2f initial_velocity,
  ...
  void SetVelocity(const Eigen::Vector2f& new_velocity);

  void RegisterKick(const Kick& kick_attempt);

  void UpdatePosition();

  void SetPose(const Eigen::Vector2f& new_pose);

  pose_2d::Pose2Df GetNextPosition();
...
#endif  // SRC_EXPERIMENTAL_SIM_OBJECTS_BALL_H_
