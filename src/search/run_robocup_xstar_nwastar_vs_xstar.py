...
def run_nwastar(num_robots):
    result = subprocess.run([str(e) for e in ['./bin/robocup_nwastar', num_robots, scenario_name, inflation, 100, seed ]], stdout=subprocess.PIPE)
    result_lines = result.stdout.decode('UTF-8').split("\n")
...
experiment_name = "xstar_vs_nwastar"
