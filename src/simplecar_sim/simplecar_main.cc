...
void SetupRoads(NetLogger* logger) {
  ...
  logger->AddLine({right, lower}, {right, upper}, 1.0, 1.0, 1.0, 1.0);
...
//   std::cout << "Collision Dist: " << distance.norm() << endl;
  ...
  // Must be at least this close to care.
//   if (distance.norm() < 750.0) {
//     // This means we are too close in the x direction
//     if (distance.x() < 750.0) {
//       return true;
//     }
//   }
...
int main(int argc, char** argv) {
  ...
  RateLoop loop(120.0);
  SimplecarController controller = SimplecarController("Simplecar", 0);
  while (is_running_) {
