...
Policy* mstar::grid_policy_ptr(const std::vector<std::vector<bool>> &world_map,
			const std::pair<int, int> &goal){
  int columns = (int) world_map[0].size();
  NP_CHECK(goal.first >= 0);
  NP_CHECK(goal.second >= 0);
  NP_CHECK(goal.first < columns);
  NP_CHECK(goal.second < static_cast<int>(world_map.size()));
  NP_CHECK(static_cast<int>(world_map.size()) > goal.first);
  NP_CHECK(static_cast<int>(world_map[goal.first].size()) > goal.second);
