...
if __name__ == "__main__":
    ...
    x = float(line[0:ind])
    y = float(line[ind+2:-1])
    ax.add_patch(plt.Circle((x,y), 90, color='g', fill=True))
    line = f.readline()
    ...
    while (line != "[FINAL PATH LENGTH]\n"): 
        ...
        if type == "rect":
            ...
            ax.add_patch(plt.Rectangle((x - width/2,y-height/2), width, height, color='r'))
        elif type == "circ" or type == "robo":
            # Circle
            radius = float(split_line[3])
            print("Radius: " + str(radius))
            ax.add_patch(plt.Circle((x,y), radius, color='r', fill=True))
        line = f.readline()
