...
search_tree_files_list = [(get_key(f), []) for f in scenerio_files]
search_tree_files_list = [sorted([f for f in files if "scenerio_{}_step".format(pair[0]) in f], key=get_key) for pair in search_tree_files_list]
...
def press(event):
    ...
    plot_search_tree(scenerio_message, search_tree_files_list[scenerio_files_index][search_tree_files_index])
    plt.xlim([-5,5])
    plt.ylim([-5,5])
    plt.axes().set_aspect('equal', 'datalim')
    plt.tight_layout()
...
def plot_search_tree(scenerio_message, search_tree_file):
    print("Search tree file:", search_tree_file)
    ...
    search_tree_message = text_format.Parse(f.read(), search_pb2.SearchTree())
    draw_prev_edges(search_tree_message.other_edge_list)
    draw_current_edge(search_tree_message.current_edge)
    #draw_closed_list(search_tree_message.closed_positions)
    print("status:", search_tree_message.status)
